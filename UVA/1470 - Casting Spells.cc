...
void manacher(const char *s, int u[], int n) {
	...
	u[0] = 0;
	for (int i = 1; i <n; ++ i) u[i] = u[2 * i - 1] / 2;
}

void build_st(int n) {
	for (int i = 0; i < n; ++ i) mx[i][0] = u[i] + i;
...
int query(int l, int r, int x) {
	...
	if (lm >= x) return query(l, mid, x);
	else if (rm >= x) return query(mid + 1, r, x);
	else return -1;
}

int main() {
	for (int i = 1; i < MAXN; ++ i) log[i] = 31 - __builtin_clz(i);
	int T; scanf("%d", &T);
	while (T --) {
		scanf("%s", st);
		int n = strlen(st);
